...
namespace mOpenCV.TrafficLight
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var h1 = src.CvtColor(ColorConversionCodes.RGB2HSV);
            var channel = Cv2.Split(h1);
            for (int i = 0; i < channel[1].Rows; i++)//遍历所有像素
            {
                for (int j = 0; j < channel[2].Cols; j++)
                {
                    channel[1].Set(i, j, 255);//改变饱和度
                }
            }
            Cv2.Merge(channel, h1);
            var h2 = h1.CvtColor(ColorConversionCodes.HSV2RGB);
            Cv2.ImShow("Saturation++", h2);

            Mat m1 = new Mat();
            Cv2.MedianBlur(h2, m1, 11); //  ksize必须大于1且是奇数
            for (var y = 0; y < m1.Height; y++)
            {
                for (var x = 0; x < m1.Width; x++)
                {
                    var color = m1.Get<Vec3b>(y, x);
                    //if (color.Item2 == 255 && color.Item1 < 100 && color.Item0 == 0) //for R
                    if (color.Item2 == 255 && color.Item1 > 100 && color.Item0 == 0) //for Y
                    //if (color.Item1 > 100 && color.Item1 == 255 && color.Item2 == 0) //for G
                    {
                        color.Item0 = 0;
                        color.Item1 = 0;
                        color.Item2 = 0;
            ...
            /*
                 ...
                 * 
                 */
            CircleSegment[] cs = Cv2.HoughCircles(m2, HoughMethods.Gradient, 1, 80, 60, 30, 30, 100);
